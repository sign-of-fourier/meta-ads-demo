...
| `modal_bo.py` | PCA helpers (`fit_pca`, `project`, `dim_bounds`, `pca_dims_for_platform`), env-var config (`MODAL_BO_API_URL`, `MODAL_BO_PCA_DIMS`, `GOOGLE_BO_PCA_DIMS`), `modal_bo_enabled()`. Re-exports `call_modal_api` and `call_modal_api_multioutput` from `quantecarlo._modal_api` — HTTP logic lives there, not here. |
...
| `cross_platform.py` | Three cross-platform functions: `run_cross_platform_bo(pairs, ..., method="modal")` — original, per-platform PCA+GPR, global EI rank; returns `(picks, group_stats)`. `run_unified_cross_platform_bo(pairs, ..., top_n=4, method="modal"|"modal_multioutput")` — `method="modal"`: **single shared PCA** on pooled 3072-dim vectors (Google zero-padded via `build_X_unified()`), single GP/Modal call; `method="modal_multioutput"`: **separate PCA per platform** (no zero-padding), calls `call_modal_api_multioutput` with `d`/`d_candidates`/`rho`, falls back to shared-PCA path on failure. Internal helpers: `_run_unified_modal_bo_multioutput` (multioutput path), `_run_group_modal_bo` (per-group single-output Modal path for `run_cross_platform_bo`). |
